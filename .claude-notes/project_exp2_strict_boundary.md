...
**Airtight attribution (closes the last gap):** both groups call `torch.manual_seed(42)` before building the model ([exp2_causal.py:89](../../experiments/exp2_causal.py#L89)), so GRU and W_action are byte-identical and frozen; the ONLY trained/differing parameter is `pred.weight`. During disconnect, obs and GRU are identical → h converges identically → the recovery difference is 100% attributable to `pred.weight` — i.e. the readout structure learned in training (Causal = decomposable: obs0 = signal-part(h) + action-part(h), both functions of h; Control = entangled: noise is external, not reconstructable from h). Because pred is ALSO frozen at test, recovery is a read-only probe of that training-learned decomposition. The naive intuition "same disconnect obs → equal recovery" misses exactly that pred is frozen at test.
